---
name: agentic-os
description: Build persistent multi-agent operating systems on Codex. Covers kernel instructions, Codex subagents, file-based memory, scheduled automation, and state management without external databases.
origin: ECC
---

# Agentic OS

Treat Codex as a persistent runtime / operating system rather than a chat session. This skill codifies the architecture used by production agentic setups: kernel instructions that route tasks to specialist subagents, persistent file-based memory, scheduled automation, and a JSON/markdown data layer.

## Codex Port Note

This repository currently provisions Codex-native pieces:

- `.codex/AGENTS.md` for kernel/project behavior
- `.codex/agents/*.toml` for specialist subagents
- `.agents/skills/*/SKILL.md` for reusable workflows
- `.codex/config.toml` for MCP and subagent limits

Do not assume Claude Code-only paths such as `.claude/commands/` exist in this repo. If a workflow needs commands, scripts, or persistent data folders, create them explicitly as part of that workflow.
...
```
project-root/
├── .codex/AGENTS.md   # Kernel: behavior, routing rules, project policy
├── .codex/agents/     # Specialist subagent definitions (TOML)
...
|---|---|---|
| Kernel (`.codex/AGENTS.md`) | Identity, routing, model policies, project rules | Git-tracked |
| Subagents (`.codex/agents/*.toml`) | Specialist identities with scoped sandbox/model settings | Git-tracked |
| Skills (`.agents/skills/*/SKILL.md`) | Reusable workflows loaded on demand | Git-tracked |
...
`.codex/AGENTS.md` is the kernel. It acts as the COO / orchestrator. Codex reads it as project behavior and uses it to steer work.

### Kernel Structure

```markdown
# .codex/AGENTS.md - Agentic OS Kernel
...
3. Spawn the corresponding Codex subagent from `.codex/agents/<name>.toml`
...
Each specialist is a standalone TOML file in `.codex/agents/`. Codex loads the relevant custom agent file when spawning a subagent.

### Subagent Definition Format

```toml
name = "dev"
description = "Software engineer for small, tested implementation tasks."
model = "gpt-5.4"
model_reasoning_effort = "medium"
sandbox_mode = "workspace-write"

developer_instructions = '''
You are a senior software engineer. You write clear, tested code.
Read data/projects/<current-project>.md and data/decisions/ when they exist.
Keep changes scoped, never commit directly to main, and run relevant tests.
MCP servers come from .codex/config.toml unless this agent overrides them.
'''
```
...
For parallel execution, explicitly ask Codex to spawn subagents and summarize their results. Keep write scopes disjoint when multiple subagents can edit files.

## Reusable Workflows

Codex workflows can be captured as repo docs, skills, or project-specific prompts. Do not assume a command file exists until it is created and documented.
...
Invoke workflows by asking Codex directly, by referencing a skill with `$skill-name`, or by spawning a named subagent. If you add command files later, document their exact location and invocation syntax in this repository.
...
Agentic OS tasks run on a schedule using external cron or another process manager, not an interactive Codex session.
...
<dict>
    ...
    <array>
        ...
        <string>Run the daily-sync workflow.</string>
    </array>
...
[Service]
Type=oneshot
ExecStart=/usr/local/bin/codex exec --cd /path/to/project "Run the daily sync workflow."
```
...
module.exports = {
  apps: [{
    ...
    script: 'codex',
    args: 'exec --cd /path/to/project "Run the daily sync workflow."',
    cron_restart: '0 8 * * *',
    autorestart: false
  }]
};
...
```markdown
# BAD - API keys in agent files or .codex/AGENTS.md
...
Keep routing declarative in `.codex/AGENTS.md` markdown tables. It is inspectable, editable, and debuggable.

## Best Practices

- [ ] `.codex/AGENTS.md` is under 200 lines and fits in context window
...
- [ ] Reusable workflows use imperative names: `daily-sync`, not `run-daily-sync`
...
- [ ] Scheduled tasks use external cron (LaunchAgent, systemd, pm2), not an interactive Codex session
- [ ] Cost tracking: log API spend per session in `data/logs/<date>-costs.json`
- [ ] One project = one Agentic OS. Do not share a single `.codex/AGENTS.md` across unrelated projects.
